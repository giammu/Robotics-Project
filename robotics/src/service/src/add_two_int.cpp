

#include "ros/ros.h"
//#include "service/AddTwoInts.h"
    //definiamo la callback
bool add(service::AddTwoInts::Request  &req, //qui attende a e b in input
         service::AddTwoInts::Response &res) //qui crea in output la somma
{
  res.sum = req.a + req.b;
  ROS_INFO("request: x=%ld, y=%ld", (long int)req.a, (long int)req.b);
  ROS_INFO("sending back response: [%ld]", (long int)res.sum);
  return true; //non pubblichiamo il risultato ma semplicemente ritorniamo true
}

int main(int argc, char **argv)
{
  ros::init(argc, argv, "add_two_ints_server");
  ros::NodeHandle n;

  ros::ServiceServer service = n.advertiseService("add_two_ints", add); //creiamo il service
  ROS_INFO("Ready to add two ints.");
  ros::spin();

  return 0;
}
